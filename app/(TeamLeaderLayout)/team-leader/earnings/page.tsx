...
interface PayoutLog {
  ...
  leaderCommission: number; // 10%
  cleanerPoolShare: number; // 40%
  completedAt: string;
  status: "CREDITED" | "PENDING_RELEASE";
}

export default function TeamLeaderEarningsPage() {
  const [logs, setLogs] = useState<PayoutLog[]>([
    {
      id: "PAY-501",
      bookingId: "CLN-2026-8890",
      customerName: "Anisur Rahman",
      serviceType: "Premium Plan Visit #4 (৳30,000/mo)",
      totalBookingPrice: 30000,
      leaderCommission: 375,
      cleanerPoolShare: 1500,
      completedAt: "Today at 12:15 PM",
      status: "CREDITED",
    },
    {
      id: "PAY-500",
      ...
      serviceType: "VIP Sofa & Carpet Wash (Custom One-Time)",
      totalBookingPrice: 8500,
    ...
    {
      id: "PAY-499",
      bookingId: "CLN-2026-8880",
      customerName: "Sabrina Rahman",
      serviceType: "Standard Plan Visit #1 (৳14,000/mo)",
      totalBookingPrice: 14000,
    ...
    <div className="space-y-8 pb-12 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-2">
          <span className="text-xs font-extrabold text-slate-500 uppercase tracking-wider block">
            This Month Total Leader Cut (10%)
          </span>
          <div className="text-2xl font-black text-[#0d274c]">৳১৫,৭৫০</div>
          <p className="text-xs font-bold text-emerald-600">From 24 Completed Visits</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-2">
          ...
          <div className="text-2xl font-black text-blue-600">৳৬৩,০০০</div>
          <p className="text-xs font-medium text-slate-500">Distributed among Team Cleaners</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xs space-y-2">
          ...
          <div className="text-2xl font-black text-slate-800">৳৭৮,৭৫০</div>
          <p className="text-xs font-medium text-slate-500">Covers kits, gear & platform net</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-xs space-y-4 p-6">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900">Per-Visit Earnings Breakdown</h2>
            <p className="text-xs text-slate-500 font-medium">প্রতিটি কাজ সম্পন্ন হওয়ার সাথে সাথেই কমিশন ওয়ালেটে ক্রেডিট হয়</p>
          </div>
            ...
            <div key={log.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-black text-[#007eff]">{log.bookingId}</span>
                  <h3 className="font-extrabold text-slate-900 text-sm">{log.customerName}</h3>
                </div>
