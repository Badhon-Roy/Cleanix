"use client";

import React from "react";
import Link from "next/link";
import { ChevronRight, Phone, Mail, MapPin } from "lucide-react";

export function FacebookIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  );
}

export function TwitterIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}

export function InstagramIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
    </svg>
  );
}

export function LinkedinIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
      <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.762-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
    </svg>
  );
}

export default function Footer() {
  return (
    <footer className="w-full bg-[#001837] text-white relative pt-12 pb-10 px-4 sm:px-6 lg:px-8 border-t border-white/10">
      <div className="container mx-auto">
        {/* Main Footer Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-12 mb-16">
          {/* Column 1: Brand Info & Contact (lg:col-span-4) */}
          <div className="lg:col-span-4">
            {/* Logo */}
            <Link href="/" className="inline-flex items-center gap-2.5 mb-6">
              <div className="w-10 h-10 rounded-full bg-[#007eff] flex items-center justify-center shadow-lg shadow-blue-500/30">
                <svg
                  viewBox="0 0 24 24"
                  className="w-6 h-6 text-white fill-none stroke-current stroke-[2.5]"
                >
                  <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14.5v-9l6 4.5-6 4.5z" />
                </svg>
              </div>
              <span className="text-white font-black text-2xl tracking-wider uppercase">
                CLAEANIX
              </span>
            </Link>

            <p className="text-slate-300 text-sm leading-relaxed mb-6">
              বাংলাদেশের ১ নম্বর অন-ডিমান্ড প্রিমিয়াম হোম ও কর্পোরেট ক্লিনিং প্ল্যাটফর্ম। আপনার বাসা এবং অফিসকে ঝকঝকে ও জীবাণুমুক্ত রাখার সহজ ডিজিটাল সমাধান।
            </p>

            {/* Contact Details */}
            <div className="space-y-3 text-slate-300 text-sm">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-[#007eff] flex-shrink-0">
                  <Phone className="w-4 h-4" />
                </div>
                <span className="font-semibold">+880 1800-253264 (+880 1800-CLEANIX)</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-[#007eff] flex-shrink-0">
                  <Mail className="w-4 h-4" />
                </div>
                <span className="font-semibold">support@cleanix.com.bd</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-[#007eff] flex-shrink-0">
                  <MapPin className="w-4 h-4" />
                </div>
                <span className="font-semibold">
                  Gulshan Avenue, Circle-2, Dhaka-1212, Bangladesh
                </span>
              </div>
            </div>
          </div>

          {/* Column 2: Quick Links (lg:col-span-2) */}
          <div className="lg:col-span-2">
            <h3 className="text-white font-extrabold text-sm tracking-wider uppercase mb-6 pb-2 border-b border-white/10 inline-block">
              QUICK LINKS
            </h3>
            <ul className="space-y-3 text-slate-300 text-sm font-medium">
              {[
                { name: "Home", href: "/" },
                { name: "About Us", href: "/about" },
                { name: "Our Services", href: "/services" },
                { name: "Our Projects", href: "/projects" },
                { name: "Contact Us", href: "/contact" },
                { name: "Pricing", href: "/#pricing" },
              ].map((item, i) => (
                <li key={i}>
                  <Link
                    href={item.href}
                    className="hover:text-[#007eff] hover:translate-x-1 transition-all duration-300 flex items-center gap-1.5 inline-flex"
                  >
                    <ChevronRight className="w-3.5 h-3.5 text-[#007eff]" />
                    <span>{item.name}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 3: Our Services (lg:col-span-3) */}
          <div className="lg:col-span-3">
            <h3 className="text-white font-extrabold text-sm tracking-wider uppercase mb-6 pb-2 border-b border-white/10 inline-block">
              OUR SERVICES
            </h3>
            <ul className="space-y-3 text-slate-300 text-sm font-medium">
              {[
                "Residential Deep Clean",
                "Commercial Office Cleaning",
                "Move-Out Cleaning",
                "Post-Construction Clean",
                "Sanitization & Disinfection",
                "Window & Glass Wash",
              ].map((service, i) => (
                <li key={i}>
                  <Link
                    href={`#service-${i}`}
                    className="hover:text-[#007eff] hover:translate-x-1 transition-all duration-300 flex items-center gap-1.5 inline-flex"
                  >
                    <ChevronRight className="w-3.5 h-3.5 text-[#007eff]" />
                    <span>{service}</span>
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 4: Newsletter Subscribe (lg:col-span-3) */}
          <div className="lg:col-span-3">
            <h3 className="text-white font-extrabold text-sm tracking-wider uppercase mb-6 pb-2 border-b border-white/10 inline-block">
              NEWSLETTER
            </h3>
            <p className="text-slate-300 text-sm leading-relaxed mb-4">
              Subscribe to get exclusive cleaning discounts, seasonal tips, and
              updates directly in your inbox.
            </p>
            <form onSubmit={(e) => e.preventDefault()} className="space-y-3">
              <input
                type="email"
                placeholder="Enter your email address..."
                className="w-full bg-[#071f40] border border-white/15 rounded-full px-5 py-3 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-[#007eff] transition-colors"
                required
              />
              <button
                type="submit"
                className="w-full bg-[#007eff] hover:bg-[#0066ee] text-white font-bold text-xs uppercase tracking-wider py-3.5 rounded-full shadow-lg shadow-blue-500/25 transition-all duration-300 hover:scale-[1.02] flex items-center justify-center gap-2"
              >
                <span>Subscribe Now</span>
                <ChevronRight className="w-4 h-4 stroke-[3]" />
              </button>
            </form>
          </div>
        </div>

        {/* Bottom Footer Bar */}
        <div className="border-t border-white/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400">
          <p>© {new Date().getFullYear()} Claeanix. All rights reserved.</p>

          {/* Social Media Icons */}
          <div className="flex items-center gap-3">
            <Link
              href="#"
              className="w-9 h-9 rounded-full bg-white/10 hover:bg-[#007eff] hover:text-white flex items-center justify-center transition-all duration-300 text-slate-300"
            >
              <FacebookIcon />
            </Link>
            <Link
              href="#"
              className="w-9 h-9 rounded-full bg-white/10 hover:bg-[#007eff] hover:text-white flex items-center justify-center transition-all duration-300 text-slate-300"
            >
              <TwitterIcon />
            </Link>
            <Link
              href="#"
              className="w-9 h-9 rounded-full bg-white/10 hover:bg-[#007eff] hover:text-white flex items-center justify-center transition-all duration-300 text-slate-300"
            >
              <InstagramIcon />
            </Link>
            <Link
              href="#"
              className="w-9 h-9 rounded-full bg-white/10 hover:bg-[#007eff] hover:text-white flex items-center justify-center transition-all duration-300 text-slate-300"
            >
              <LinkedinIcon />
            </Link>
          </div>

          {/* Legal Links */}
          <div className="flex items-center gap-4 font-medium">
            <Link href="#privacy" className="hover:text-white transition-colors">
              Privacy Policy
            </Link>
            <span>•</span>
            <Link href="#terms" className="hover:text-white transition-colors">
              Terms of Service
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
