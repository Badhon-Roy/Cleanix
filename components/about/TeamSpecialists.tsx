      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-20 relative z-10">
          {team.map((member) => (
            <div
              key={member.id}
              className="bg-[#f4f6f9] rounded-3xl p-5 border border-slate-200/80 transition-all duration-300 flex flex-col justify-between group hover:border-[#007eff]/50 hover:bg-white shadow-xs hover:shadow-lg"
            >
              <div className="relative w-full h-[240px] rounded-[24px_48px_24px_24px] overflow-hidden mb-4 bg-slate-200">
                <Image
                  src={member.image}
                  alt={member.name}
                  fill
              ...
              <div className="px-2 pb-2 space-y-1">
                <div className="flex items-center justify-between gap-2">
                  <h3 className="text-[#001837] font-extrabold text-lg">
                    {member.name}
                  </h3>
                  {member.nidVerified && (
